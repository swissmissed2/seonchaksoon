...
- 부하 증가 시 요청이 락을 기다리며 쌓이고, 톰캣 스레드풀이 전부 블로킹되면서 503 응답 발생(애플리케이션이 만든 응답이 아니라 컨테이너 레벨의 서비스 붕괴)
...
#### Optimistic Lock
- 버전 컬럼 기반 충돌 감지
- 락 대기 없이 충돌 시 즉시 실패 → 애플리케이션 레벨에서 최대 100회까지 재시도(Thread.yield)
- 재시도가 짧고 가벼운 대신, 부하가 커지면 100회를 다 써도 통과하지 못하는 경우가 늘어남
- 낮은 부하에서는 높은 처리량과 낮은 지연 시간
- 부하가 커질수록 재시도 실패(타임아웃)율이 상승 — 비즈니스 요구에 따라 적합성 달라짐
...
- 테스트 시간: 10초
- 쿠폰 발급 수량(limit): 100
- 모든 전략은 동일 조건에서 테스트 수행 — 전략 전환 시마다 `/coupons/reset/{eventKey}`로 DB 발급 기록과 Event 카운트, Redis 락 키를 초기화한 뒤 측정
